...
import cv2
from common import gameScreenVT,gameScreenTPL,gameScreenTPLM,gameScreen
...
#Fare İşlemleri
def on_move(x, y):
  x = Thread(target=gameScreenTPLM,args=(fareEgitimYolu,x,y))
  x.start()
...
def on_click(x, y, button, pressed):
    ...
    th = Thread(target=gameScreenTPLM,args=(fareEgitimYolu, x, y, action, key))
    th.start()
...
def on_press(key):
    ...
    th = Thread(target=gameScreenTPLM,args=(klavyeEgitimYolu, "-1", "-1", "1", tus))
    th.start()
    if tus == "x":
          sys.exit()
def on_release(key):
     ...
     tus = str(key).strip("'") if len(str(key).strip("'"))<4 else str(key)[4:]
     th = Thread(target=gameScreenTPLM,args=(klavyeEgitimYolu, "-1", "-1", "2", tus))
     th.start()
